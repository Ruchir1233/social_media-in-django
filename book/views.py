...
def upload(request):
    if request.method=='POST':
        print("i have run here what abput ypu? love?")
    ...
    if request.method == 'POST':
        ...
        if pass1 == pass2:
            if User.objects.filter(email=email).exists():
                messages.info(request,'pacho same email km lakhe che bhai?')
                return redirect('signup')
            elif User.objects.filter(username=username).exists():
                messages.info(request,'username biju nakho please')
                return redirect('signup')
            else:
            ...
            messages.info(request,'password match nathi thato bhai')
            return redirect('signin')
...
def signin(request):
    if request.method=='POST':
        ...
        if user is not None:
            ...
            messages.info(request,'kai khotu che bhai')
            return redirect('signin')
